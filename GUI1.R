...
binomial_mod=function(){
  ...
  tk2tip(K.entry,"Strike price. It must be positive.")  #create tooltips
  ...
  reset.but=tkbutton(tt,text="Reset",command=reset,cursor="hand1",width=5)
  calculate=function(){
    ...
    if(S>0 & K>0 & T>0 & r>0 & n>0){ 
      ...
      quit2.but=tkbutton(tt2,text="Quit",command=function(){tkdestroy(tt2)},cursor="hand2",width=7,fg="white")
      
      title2=tkfont.create(size=14,weight="bold",family="Tahoma")
      txt2=tkfont.create(size=12,family="Times New Roman",weight="bold") #create new fonts
      label2=tklabel(tt2,text="Results")
      txt_results=tkfont.create(size=10,family="Cambria")
      button_txt2=tkfont.create(size=10,family="Helvetica",weight="bold")
      
      tkconfigure(label2,font=title2) #config label2
      tkconfigure(quit2.but,bg="#FE2E2E",font=button_txt2) #config quit button

      am_eu=ifelse(opt=="am",TRUE,FALSE)
      b=bino2(S,K,T,r,n,sigma,TRUE,am_eu)
          if(am_eu){ #binomial models
            put=b
            call=bino2(S,K,T,r,n,sigma,FALSE,am_eu)
            tkgrid(label1,columnspan=3,pady=9)
            tkgrid(tklabel(tt2,text="American Put",font=txt2),tklabel(tt2,text=put,font=txt_results),pady=3,padx=1)
            ...
            tkgrid(label1,columnspan=3,pady=9)
            tkgrid(tklabel(tt2,text="Put",font=txt2),tklabel(tt2,text=put,font=txt_results),pady=3,padx=1)
            ...
            font=txt_results),pady=9,padx=8)
            tkgrid(quit2.but,columnspan=3,pady=9)
          }
        }
    else{
  ...
  calculate.but=tkbutton(tt,text="Calculate",command=calculate,cursor="hand2",width=9)
  quit.but=tkbutton(tt,text="Quit", command=function(){tkdestroy(tt)},cursor="hand2",fg="white",width=5)
  
  txt=tkfont.create(size=11,family="Times New Roman")
  title=tkfont.create(size=14,weight="bold",family="Tahoma")
  ...
  label1=tklabel(tt,text="Write your variables")
  ...
  tkconfigure(S.entry,font=entry_txt)
